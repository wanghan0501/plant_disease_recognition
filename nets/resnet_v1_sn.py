...
"""
Code from github:
https://github.com/switchablenorms/Switchable-Normalization/blob/master/models/resnet_v1_sn.py
"""
...
class ResNetV1SN(nn.Module):
    ...
    def __init__(self, block, layers, num_classes=1000, using_moving_average=True, last_gamma=True, drop_prob=0.5):
        self.inplanes = 64
    ...
    def _make_layer(self, block, planes, blocks, stride=1, last_gamma=True):
        ...
        if stride != 1 or self.inplanes != planes * block.expansion:
            downsample = nn.Sequential(
                ...
                sn.SwitchNorm(planes * block.expansion, using_moving_average=self.using_moving_average),
            )
        ...
        for i in range(1, blocks):
            layers.append(
                block(self.inplanes, planes, using_moving_average=self.using_moving_average, last_gamma=last_gamma))
